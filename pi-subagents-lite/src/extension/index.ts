...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
	...
	type ChatTarget = { kind: "main" } | { kind: "agent"; agent: string };
	let chatTarget: ChatTarget = { kind: "main" };

	const formatChatTarget = () => chatTarget.kind === "main" ? "main" : `subagent:${chatTarget.agent}`;

	const refreshChatTargetUi = (ctx: ExtensionContext) => {
		if (!ctx.hasUI) return;
		ctx.ui.setStatus("subagent-chat", `chat → ${formatChatTarget()}`);
	};

	const setChatTarget = (ctx: ExtensionContext, target: ChatTarget) => {
		chatTarget = target;
		refreshChatTargetUi(ctx);
		ctx.ui.notify(`Chat target: ${formatChatTarget()}`, "info");
	};

	const chooseChatTarget = async (ctx: ExtensionContext) => {
		const agents = discoverAgents(ctx.cwd, "both").agents
			.map((agent) => agent.name)
			.sort((a, b) => a.localeCompare(b));
		const choices = ["main", ...agents.map((agent) => `subagent:${agent}`)];
		const choice = await ctx.ui.select("Send future chat messages to:", choices);
		if (!choice) return;
		if (choice === "main") {
			setChatTarget(ctx, { kind: "main" });
			return;
		}
		setChatTarget(ctx, { kind: "agent", agent: choice.slice("subagent:".length) });
	};

	pi.registerShortcut("ctrl+shift+a", {
		description: "Toggle chat target between main and subagents",
		handler: chooseChatTarget,
	});

	pi.registerCommand("chat-target", {
		description: "Choose whether chat goes to the main agent or a subagent",
		handler: async (args, ctx) => {
			const target = args.trim();
			if (!target) {
				await chooseChatTarget(ctx);
				return;
			}
			if (target === "main") {
				setChatTarget(ctx, { kind: "main" });
			...
			const agent = target.startsWith("subagent:") ? target.slice("subagent:".length) : target;
			const agents = new Set(discoverAgents(ctx.cwd, "both").agents.map((entry) => entry.name));
			if (!agents.has(agent)) {
				ctx.ui.notify(`Unknown subagent: ${agent}`, "error");
				return;
			}
			setChatTarget(ctx, { kind: "agent", agent });
		},
	});

	pi.on("input", (event) => {
		if (chatTarget.kind === "main") return { action: "continue" };
		if (event.source === "extension") return { action: "continue" };
		const text = event.text.trim();
		if (!text || text.startsWith("/")) return { action: "continue" };
		return {
			action: "transform",
			text: `Route this user message to subagent '${chatTarget.agent}' using the subagent tool. Do not answer directly before the tool call. After the subagent returns, relay the result concisely.\n\nUser message:\n${event.text}`,
		};
	});
